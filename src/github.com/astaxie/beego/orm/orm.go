...
func (o *orm) Using(name string) error {
	...
	} else if v, ok := mysqlConStr[name]; ok == true {
		RegisterDataBase(name, "mysql", v) //fussen  reconnect to db server
		if al, ok := dataBaseCache.get(name); ok {
			o.alias = al
			if Debug {
				o.db = newDbQueryLog(al, al.DB)
...
// return a raw query seter for raw sql string. Fussen dylan
func (o *orm) RawJSON(query string, json config.LJSON) RawSeter {
	var args []interface{}
	posKey := make(map[int]string)
	var sortPos []int
	sql := strings.ToUpper(query)
	params, _ := json.Interface().(map[string]interface{})
...
// return table cols Fussen dylan
func (o *orm) GetCols(table string) (result []string) {
	mi, _ := modelCache.get(table)
	if mi == nil {
		return result
	}
...
// return table PrimaryKey Fussen dylan
func (o *orm) GetPK(table string) (result []string) {
	mi, _ := modelCache.get(table)
	if mi == nil {
		return result
	}
	if mi.fields.pk != nil {
		result = append(result, mi.fields.pk.column)
	}
	if mi.fields.pk1 != nil {
		result = append(result, mi.fields.pk1.column)
	}
	return
}
