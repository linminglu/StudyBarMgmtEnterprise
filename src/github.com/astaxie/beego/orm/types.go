...
type Ormer interface {
	...
	RawJSON(query string, json config.LJSON) RawSeter
	Driver() Driver
	GetCols(table string) []string
...
type DBCacher interface {
	Save(sql string, args string, txt string)
	Load(sql string, args string) (string, bool)
}
...
type RawSeter interface {
	...
	// query data to []map[string]interface fussen dylan
	// see QuerySeter's Values
	ValuesJSON(container *config.LJSON, cols ...string) (int64, error)
	// query data to json fussen dylan
	// see QuerySeter's Values
	ValuesJC(container *config.LJSON, cacher DBCacher, cols ...string) (int64, error)
