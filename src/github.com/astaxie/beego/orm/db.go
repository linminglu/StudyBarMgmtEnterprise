...
func (d *dbBase) Read(q dbQuerier, mi *modelInfo, ind reflect.Value, tz *time.Location, cols []string, isForUpdate bool) error {
	...
	if len(cols) > 0 {
		...
		pkColumn1, pkValue1, ok1 := getExistPk1(mi, ind)
		if ok1 == true {
			whereCols = append(whereCols, pkColumn1)
			args = append(args, pkValue1)
		}
	}
...
func (d *dbBase) Update(q dbQuerier, mi *modelInfo, ind reflect.Value, tz *time.Location, cols []string) (int64, error) {
	...
	pkName1, pkValue1, ok1 := getExistPk1(mi, ind)

	andSql := ""
	if ok1 == true {
		andSql = fmt.Sprintf(" AND %s%s%s = ?", Q, pkName1, Q)
		setValues = append(setValues, pkValue1)
	}

	query := fmt.Sprintf("UPDATE %s%s%s SET %s%s%s = ? WHERE %s%s%s = ?%s", Q, mi.table, Q, Q, setColumns, Q, Q, pkName, Q, andSql)
