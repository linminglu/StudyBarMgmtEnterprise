...
func (o *rawSet) Exec() (result sql.Result, err error) {
	query := o.query
	o.orm.alias.DbBaser.ReplaceMarks(&query)
	args := getFlatParams(nil, o.args, o.orm.alias.TZ)
	queryLower := strings.ToLower(query)
	//by liangyuan.deng insert or update操作db_koala数据库，每个表必须带上lastoperator=‘WEB’
	if (strings.Contains(o.orm.alias.Name, "db_koala") && strings.Contains(queryLower, "delete") == false) || strings.Contains(queryLower, "insert into db_koala.") || strings.Contains(queryLower, "update db_koala.") {
		if strings.Contains(queryLower, "lastoperator") == false && strings.Contains(queryLower, "db_flybear.") == false && strings.Contains(queryLower, "db_image.") == false {
			logs.Error("DB.Exec error: insert or update db_koala must set field lastoperator='WEB', sql: " + query)
			o.paramsLog(args)
		}
	}
	result, err = o.orm.db.Exec(query, args...)
	//by Fussen dylan
	if err != nil {
		logs.Error("<RawSet> Exec %s,sql: %s", err.Error(), o.query)
		o.paramsLog(args)
	}
	return result, err
}
...
//fussen minfeng.chen 在日志显示数据库参数
func (o *rawSet) paramsLog(args []interface{}) {
	var msg string
	for _, arg := range args {
		...
		case reflect.String:
			msg += val.String() + "|"
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			msg += fmt.Sprintf("%d", val.Int()) + "|"
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
			msg += fmt.Sprintf("%d", val.Uint()) + "|"
		case reflect.Float32:
			xx, _ := StrTo(ToStr(arg)).Float64()
			msg += fmt.Sprintf("%f", xx) + "|"
		case reflect.Float64:
			msg += fmt.Sprintf("%f", val.Float()) + "|"
		case reflect.Bool:
			msg += fmt.Sprintf("%t", val.Bool()) + "|"
...
//fussen dylan 参数字符串
func (o *rawSet) SprintArgs() string {
	var msg string
...
func (o *rawSet) readValues(container interface{}, needCols []string) (int64, error) {
	...
	if err != nil {
		//by Fussen dylan
		logs.Error("<RawSeter> readValues %s , sql: %s", err.Error(), query)
		o.paramsLog(args)
		return 0, err
	}
	...
	for rs.Next() {
		...
		case 1:
			...
			for _, i := range indexs {
				...
				if value.Valid {
					params[strings.ToLower(cols[i])] = value.String
				} else {
					params[strings.ToLower(cols[i])] = ""
				}
			}
		...
		case 4:
			...
			for _, i := range indexs {
				...
				value := reflect.Indirect(reflect.ValueOf(ref)).Interface().(sql.NullString)
				if value.Valid {
					item.Set(strings.ToLower(cols[i])).SetString(value.String)
				} else {
					item.Set(strings.ToLower(cols[i])).SetString("")
				}
			}
		}
	...
	//fussen minfeng.chen 计算数据库处理时间，如果超时将把sql显示出来
	si := time.Since(tb)
	tlen := si.Seconds()
	if tlen >= 2 {
		msg := fmt.Sprintf("sql 耗时:%.3f秒,%s", tlen, query)
		logs.Warn(msg)
		o.paramsLog(args)
	}
	return cnt, nil
...
// query data to LJSON by fussen dylan
func (o *rawSet) ValuesJSON(container *config.LJSON, cols ...string) (int64, error) {
	return o.readValues(container, cols)
}

// query data to LJSON by fussen dylan
func (o *rawSet) ValuesJC(container *config.LJSON, cacher DBCacher, cols ...string) (int64, error) {
	txt, state := cacher.Load(o.query, o.SprintArgs())
	if state == true {
		err := container.Load(txt)
		if err == nil {
			return int64(container.ItemCount()), nil
		}
	}
	res, error := o.readValues(container, cols)
	txt, _ = container.ToString()
	cacher.Save(o.query, o.SprintArgs(), txt)
	return res, error
}
