...
func GetTimeEx(times string) time.Time {
	if "" == times {
		return time.Now()
	}
	loc, _ := time.LoadLocation("Local")
	parse, _ := time.ParseInLocation("20060102150405", times, loc)
	return parse
}

//校验参数  keys 按顺序传入参数名称
func CheckSign(param config.LJSON, keysecret string, keys ...string) bool {
	if len(keys) == 0 {
		return false
	}
	signature_str := ""
	for _, key := range keys {
		signature_str += param.Get(key).String()
	}
	...
	signature_str += oprdate
	fmt.Println(signature_str)

	signature := param.Get("signature").String()
	...
	fmt.Println(signstr)
	beego.Info(keysecret)
	beego.Info(signature_str)
	beego.Info(signstr)
