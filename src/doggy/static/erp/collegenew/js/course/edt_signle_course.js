var signledetail_pcom = {
    delimiters: ['<{', '}>'],
    template: "#edt_signlecourse_id",
    data: function () {
        var _that = this;
        return {
            showVodAlert:false,
            ...
            comment: '',
            fee: '',
            status:"",    //直播状态 0 直播暂未开始 1 直播中 2 直播结束 3 直播禁止
            istovod:"1",   //是否启用点播
            videoid:"",   //点播视频id
            playurl:'',   //播流地址
            vodPlayList:[], //点播列表
    ...
    components: {
        ...
        "uploadVideo":uploadvodvideo_pcom,
    },
    computed: {
        userData() {
            return getUserData()
    ...
    methods: {
        refreshVideo(){
            this.requestPlayUrl(this.videoid,true);
        },
        ...
        cancelVideo(val){
            this.showVodAlert = val;
        },
        chooseVideo(videoid){
            this.showVodAlert = false;
            this.videoid = videoid;
            this.video = "";
            // video
            this.requestPlayUrl(videoid,true)
        },
        getVodList(){
            var _that = this;
            _that.vodPlayList = [];
            var param = {signleid:this.signleid};
            $.ajax({
                data: param,
                url: '/butlerp/business/getvodlist',
                dataType: 'json',
                type: 'post',
                success: function (json) {
                    if (json.code == 1) {
                       ...
                       if(records&&records.length>0){
                        for(let i = 0;i<records.length;i++){
                            var item = records[i];
                            _that.getVodPlayUrl(item.videoid);
                        }
                       }
                    } else {
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.code == 1) {
                      var playList = json.list;
                      var item = playList[playList.length-1];
                      _that.vodPlayList.unshift(item.playurl);
                    } else {
                        jQuery.postFail("", json.info);
                    }
                }
        ...
        getPlayUrl(){
            ...
            $.ajax({
                data: param,
                url: '/butlerp/business/getplayurl',
                dataType: 'json',
                type: 'post',
                success: function (json) {
                    if (json.code == 1) {
                      _that.playurl = json.list.m3u8;
                      console.log(json)
                    } else {
                        jQuery.postFail("", json.info);
                    }
        ...
        saveData() {
            ...
            var param = {
                ...
                videoid:_that.videoid,
                istovod:_that.istovod
            }
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.code == 1) {
                        ...
                        _that.videoid = course.videoid;
                        _that.istovod = course.istovod;
                        _that.status = course.status;
                        ...
                        if (type == 0) {//幻灯片
                            ...
                            if(_that.istovod == "1"){
                                if(_that.videoid&&_that.videoid.length>0){
                                    _that.refreshVideo()
                                }
                               
                                _that.requestPlayUrl(_that.videoid)
                            }else{
                                if (ppt instanceof Array) {
