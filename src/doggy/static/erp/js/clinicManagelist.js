...
function datalist(pageobj){
    ...
    jQuery.loading();
    $.post("/butlerp/product/CliAndProYesRelation",parmas,function(json){
...
function comdata(data){
    var html='<thead id="thead_1"><tr><th> </th><th>序号</th><th>管家号</th><th>门诊名称</th><th>操作</th>/tr></thead><tbody>';
    if(!data){
        $("#TablelistPage").hide();
        html+='<tr class="no_data"><td colspan="5"></td></tr></tbody>';
    }else{
        $("#TablelistPage").show();
        $.each(data,function(k,v){
            k=(k+1)<10?"0"+(k+1):(k+1);
            html+='<tr>';
                html+='<td><input type="checkbox" data-denid="'+v.dentalid+'" data-cid="'+v.clinicid+'" data-cname="'+v.clinicname+'" /></td><td>'+k+'</td><td>'+v.dentalid+'</td><td>'+v.clinicname+'</td><td><span onclick="removeClinic(\''+v.dentalid+'\',\''+v.clinicid+'\',\''+v.clinicname+'\')">移除</span></td>';
            html+='</tr>';
        });
    }
    html+="</tbody>";
