var infoid=getUrlParam("infoid"),s=getUrlParam("s"),_url="";
$(function(){
    viewdata();//获取数据填充页面
...
function viewdata(){
    jQuery.loading();
    $.post("/butlerp/clinic/finishpagedata",{infoid:infoid},function(json){
        if(s==1){
            document.title="完善资料";
        }else{
            document.title="修改资料";
...
//						if(city.indexOf("城市")==-1||!city_listApi){
...
//						if(area.indexOf("地区")==-1||area.indexOf("区域")==-1){
...
function resubdata(){
    ...
    if(s==1){
        _url="/butlerp/clinic/commitapply";//完善提交资料
    }else{
        _url="/butlerp/clinic/modifyapply";//修改资料
