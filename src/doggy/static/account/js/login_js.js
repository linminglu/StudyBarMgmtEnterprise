 ...
 *------------------------------*/
 $(document).ready(function(e) {
	...
	$(".login_form").eq(0).show();
	//点击切换登陆框
	$("#mobile_login").on("click",function(){
		// $("h2 li[data-focus='1']").attr("data-focus","0");
		// $("#staff_login").attr("data-focus","0");
		// $("#mobile_login").attr("data-focus","1")
	...
	$("#staff_login").on("click",function(){
		// $("#form1 h2 li[data-focus='1']").attr("data-focus","0");
		// $("#mobile_login").attr("data-focus","0");
		// $("#staff_login").attr("data-focus","1")
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if (data.code==0) {
                ...
                $("#error_w").hide();
				if(data.list.cliniccount==0){	//未关联诊所
					window.location.href="/member/regclinicisel";
					return false;
				}
				if(data.list.userstatus==0){   //首次登录
					window.location.href="/member/activperaccount";
					return false;
				}
				if(data.list.samepassword==0){ //密码不一致
		...
		$.ajax({
			...
			success:function(data){
				...
				if (data.code==0) {
					...
					if(data.list.cliniccount>0){   //绑定手机
						if(data.list.samepassword==0){    //密码不一致
							window.location.href="/member/blindclinic";
						}else{   //密码一致
							window.location.href="/index";
									...
									if(json.code==0){
										...
										$.ajax({    //绑定手机
											...
											data:{
												...
												clinicid:data.list.clinicid,
												koalaid:data.list.koalaid
											},
											beforeSend: function(){},
											success: function(json){
												//console.log(json);
