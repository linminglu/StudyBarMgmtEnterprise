...
$.fn.yayigj_plus_date_hv = function(options) {
	...
	var 	ops=$.extend(true,{},my_params, options),
	 ...
	 var createUI=function(){		 		
	 	//alert(ops.defaulttxt);
		//console.log('ops.defaulttxt=',ops.defaulttxt);
		...
		riht_obj.on("click",function(){
		 ...
		 $("#"+jqID+"_title").yayigj_date_ymSel({callBackFunc:ops.callBackFunc,topObj:that.attr("id")});				
	 }		
...
$.fn.yayigj_date_ymSel = function(options) {
	...
	var init=function(){			
		that.on(ops.event,function(){
			var defaultVal='';
			if(ops.topObj!=null){
					defaultVal=$("#"+ops.topObj).text().replace('月','').split('年');					
					var _y=parseInt(defaultVal[0]),_m=parseInt(defaultVal[1]), _row=0,_col=0;										
					if(_m%4==0){
						_row=_m/4;
					}else{
						_row=Math.floor(_m/4)+1;
					}		
				_col=_m-(_row-1)*4;
				$("#"+that.attr('data-plusid')+"_table tr:eq("+(_row-1)+") td:eq("+(_col-1)+")").addClass('td_is_sel_ed');		
				console.log($("#"+that.attr('data-plusid')+"_table tr:eq("+(_row-1)+") td:eq("+(_col-1)+")"));	
			}
			$("#"+myid).css({
	...
	var createCSS=function(){
		if(document.getElementById(cssQz)){								
			...
			cssStr+='.td_is_sel_ed{ background-color:#F2F5FC !important; color:#849FE7}';
			var nod=$("<style type='text/css' id='"+cssQz+"'>"+cssStr+"</style>");
			$("head").append(nod);
		} 					
	};		
