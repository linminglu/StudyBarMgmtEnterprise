...
jQuery.fn.extend({
    calendar: function(c) {
        ...
        q = today.getDate(),
        k = "";
        k += "<div id='" + c.controlId + "'class='calendar "+ c.cls+"'>";
        k += "  <div class='calMain'>";
        ...
        var clickFun = function() {
            ...
            if( n.hasClass('active') ){return false;}            
            if ($("#" + c.controlId + ":hidden").length != 0) {
                n.addClass('active');
                $(".calendar").hide();
                var a = $("#" + c.controlId);
                var b = F(n[0]);
                var d = F(n[0]).x + Number(n[0].clientLeft)-1;
                b = b.y + Number(n[0].clientTop) + Number(n[0].clientHeight)-1;
                var popup = 0;
                if(c.cls == '_popup'){popup=1;}
                b = $(n[0]).offset().top + $(n[0]).outerHeight();
                d = $(n[0]).offset().left;
				a.removeClass('r');
				//判断日历左右方向
                if( d + a.outerWidth()>$(window).width()){
                    d = d - (a.outerWidth()-n.outerWidth());
                    a.addClass('r');
                }
				//判断日历上下方向
				if( b + a.outerHeight()>$(window).height()){
                    b = b - (a.outerHeight()+n.outerHeight());
                    a.addClass('t');
					n.addClass('bordertop')
                }else{
					a.removeClass('t');
					n.removeClass('bordertop')
				}
                ...
                // },
                // c.speed);
                a.css({width:'280px',height:'240px'}).fadeIn(100);
                a.bind("selectstart",
                function() {
                    return false
                }).bind("mousedown",
        ...
        if( c.delegate == false){
            n.bind("click",clickFun);
        }else{
            $(''+c.delegate).on("click",'#'+n.attr('id'),clickFun);
        }
       $(document).mouseup(function(a) {
           if (($(a.target).parentsUntil("#" + c.controlId).parent().length == 0 || $(a.target).parentsUntil("#" + c.controlId).parent()[0].id != c.controlId)){ $("#" + c.controlId).fadeOut(100,function(){n.removeClass('active bordertop');c.end()});}
       })
    }
});
